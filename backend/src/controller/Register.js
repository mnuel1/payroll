...
const Register = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        const userExists = await connection.mainConnection.query(
            'SELECT COUNT(*) as count FROM tbl_users WHERE email_address = ? FOR UPDATE',
            [email]
        );
        ...
        await connection.mainConnection.query(
            'INSERT INTO tbl_users (user_num, firstname, middlename, lastname, birthdate, contact, tin, registered_address, zip_code, foreign_address, email_address, password, signature, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ...
        const userDatabasePool = await connection.getUserDatabaseConnection(usernameFromEmail);
        ...
        await userDatabasePool.query(`USE ${userDatabaseName}`).then(async () => {
            try {
                ...
                // Handle errors related to table creation or data insertion
